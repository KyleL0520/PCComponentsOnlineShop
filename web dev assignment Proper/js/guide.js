...
function ready() {
    ...
    for (var i = 0; i < selectElements.length; i++) {
        selectElements[i].addEventListener('change', updateAddToCartButton);
        // Initially, hide the "Add to Cart" button
        updateAddToCartButton({ target: selectElements[i] });
    }
...
function updateAddToCartButton(event) {
    ...
    var addToCartButton = select.closest('.items').querySelector('.shop-item-button');
...
function addToCartClicked(event) {
    ...
    var shopItem = button.closest('.items');
    var title = shopItem.querySelector('.compType span').innerText;
    var price = shopItem.querySelector('.shop-item-price').innerText;
    var imageSrc = shopItem.querySelector('.shop-item-image').src;
    if (shopItem.querySelector('select').value !== 'empty') {
...
function addItemToCart(title, price, imageSrc) {
    ...
    var cartRowContents = `
        <div class="cart-item cart-column">
            <img class="cart-item-image" src="${imageSrc}" alt="Item Image" width="100" height="100">
...
function updateCartTotal() {
    ...
    for (var i = 0; i < cartRows.length; i++) {
        ...
        var priceText = priceElement.innerText.replace('RM', '').replace(',', '');
